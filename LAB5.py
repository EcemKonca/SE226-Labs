...
def rep_with_other(l):
    new_list = list(l)
    random_letter = random.randint(0, len(new_list))
    new_list[random_letter] = new_list[random_letter].upper()
    z = "".join(new_list)
    return z
...
while True:
    user_number = int(input("Enter the value between 3 and 7: "))
    if 3 <= user_number <= 7:
        password = pass_Gen()
...
print("One letter is upper version of the password:", rep_with_other("".join(password_list)))
print("Final version of the password:", swap_letters("".join(password_list)))
print(search_letter("".join(password_list), 'e'))
